...
describe("MatchSetupWizard Component", () => {
  ...
  it("should treat HTTP 429 as transient during online CatchMatch and fallback to syncQueue", async () => {
    const consoleWarnSpy = vi
      .spyOn(console, "warn")
      .mockImplementation(() => {});
    const handleQuickStart = vi.fn().mockResolvedValue(undefined);
    ...
    await waitFor(() => {
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        "Catch match API call failed with retryable HTTP 429, fallback to syncQueue:",
        expect.anything(),
      );
      expect(db.syncQueue.put).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("should enqueue compensating DELETE item into syncQueue when online uncatch fails with HTTP 408 timeout", async () => {
    const consoleWarnSpy = vi
      .spyOn(console, "warn")
      .mockImplementation(() => {});
    const handleQuickStart = vi
    ...
    vi.mocked(apiClient.post)
      .mockResolvedValueOnce({ id: "match-123" })
      .mockResolvedValueOnce({});
    vi.mocked(apiClient.delete).mockRejectedValueOnce({ status: 408 });
    vi.mocked(apiClient.get).mockResolvedValueOnce(mockMatch);
    vi.mocked(teamService.getTeamById)
      .mockResolvedValueOnce(mockHomeTeam)
      .mockResolvedValueOnce(mockGuestTeam);
